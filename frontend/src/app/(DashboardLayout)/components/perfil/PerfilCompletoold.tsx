// Código React completo del Perfil con documentación, historial y QR
"use client";
...
export default function PerfilCompleto({ dni }: { dni: string }) {
  ...
  useEffect(() => {
    const fetchPerfil = () => {
      fetch(`http://localhost/adeco/api/get_perfil.php?dni=${dni}`)
        .then((res) => res.json())
        .then((data) => {
    ...
    fetchPerfil();
    const i = setInterval(fetchPerfil, 150000);
  ...
  const guardarCambios = () => {
    fetch("http://localhost/adeco/api/update_perfil.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(perfil),
    })
      .then((res) => res.json())
      .then((data) => alert(data.success ? "Perfil actualizado" : "Error al guardar"));
  };
        ...
        <Grid item xs={12} md={4}>
          <Card><CardContent>
            {perfil.fotoPerfilUrl ? (
              <Avatar src={`http://192.168.0.85/adeco/api/${perfil.fotoPerfilUrl}?t=${Date.now()}`} sx={{ width: 120, height: 120, margin: "auto" }} />
            ) : (
          ...
          <Card sx={{ mt: 2 }}><CardContent>
            {perfil.fotoVehiculoUrl ? (
              <Image src={`http://192.168.0.85/adeco/api/${perfil.fotoVehiculoUrl}?t=${Date.now()}`} alt="Vehículo" width={200} height={120} style={{ borderRadius: 8, display: "block", margin: "auto" }} />
            ) : (
        ...
        <Grid item xs={12} md={8}>
          <Card><CardContent>
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}><TextField label="Teléfono" fullWidth value={perfil.telefono} onChange={(e) => setPerfil({ ...perfil, telefono: e.target.value })} /></Grid>
              <Grid item xs={6}><TextField label="Proveedor" fullWidth value={perfil.proveedor} onChange={(e) => setPerfil({ ...perfil, proveedor: e.target.value })} /></Grid>
              <Grid item xs={6}><TextField label="Estado" fullWidth value={perfil.activo} onChange={(e) => setPerfil({ ...perfil, activo: e.target.value })} /></Grid>
              <Grid item xs={6}><TextField label="Permiso hasta" type="date" fullWidth value={perfil.permisoHasta} onChange={(e) => setPerfil({ ...perfil, permisoHasta: e.target.value })} /></Grid>
              <Grid item xs={12}><TextField label="Observaciones" fullWidth multiline value={perfil.obs} onChange={(e) => setPerfil({ ...perfil, obs: e.target.value })} /></Grid>
            </Grid>
          </CardContent></Card>
                ...
                <Box key={index} display="flex" alignItems="center" gap={1} borderBottom="1px solid #eee" py={1} bgcolor={bg}>
                  <FormControl><InputLabel>Tipo</InputLabel>
                    <Select value={doc.tipo} onChange={(e) => {
                      const docs = [...perfil.documentacion];
                      docs[index].tipo = e.target.value;
                      setPerfil({ ...perfil, documentacion: docs });
                  ...
                  <Button variant="outlined" onClick={() => generarQR(doc.tipo, index)}>📎</Button>
                  <Button color="error" onClick={() => {
                    const docs = perfil.documentacion.filter((_, i) => i !== index);
                    setPerfil({ ...perfil, documentacion: docs });
                  }}>🗑</Button>
                </Box>
              );
            })}
            <Box mt={2}><Button variant="outlined" onClick={() => setPerfil({ ...perfil, documentacion: [...perfil.documentacion, { tipo: "", estado: "vigente", vencimiento: "" }] })}>+ Agregar documento</Button></Box>
          </CardContent></Card>
