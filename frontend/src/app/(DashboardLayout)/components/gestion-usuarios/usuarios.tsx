...
const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost/adeco/api";
const API_URL_SOCKET = process.env.NEXT_PUBLIC_SOCKET_API_URL || "http://localhost:5000";
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} sm={6}>
              <TextField
                label="LLave"
                InputProps={{
                  startAdornment: <VpnKeyIcon sx={{ mr: 1, color: 'grey.500' }} fontSize="small" />,
                }}
                fullWidth
                value={currentRow.Id_Key || ''}
                onChange={(e) => setCurrentRow({ ...currentRow, Id_Key: e.target.value })}
              />
            </Grid>
            {/* TAG CON LECTOR */}
            <Grid item xs={6} sm={6} sx={{ display: 'flex', alignItems: 'center' }}>
              <TextField
                ...
                onChange={async (e) => {
                  ...
                  // --- lógica original para control de tag manual/lectura
                  if (originalTag === '') {
                    setCurrentRow((prev) => ({
                      ...prev,
                      tag: newValue,
                ...
                disabled={leyendo}
                helperText={leyendo ? "Esperando lectura de antena..." : "Puede escribirlo manual o leerlo de la antena."}
              />
              <IconButton
                sx={{
                  ml: 1,
                ...
                onClick={async () => {
                  setLeyendo(true);
                  try {
                    const res = await fetch(`${API_URL_SOCKET}/leer-antenna`);
            ...
            <Grid item xs={12}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  startAdornment={<BusinessIcon fontSize="small" sx={{ mr: 1, color: 'grey.500' }} />}
                  onChange={(e) =>
                    setCurrentRow({
                      ...currentRow,
