...
const ProviderCRUD = () => {
  ...
  const fetchRows = async () => {
    const response = await fetch(`API_URL/adeco/api/get-providers.php`);
  ...
  const handleDelete = async (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`API_URL/adeco/api/delete-provider.php?id=${id}`, {
            method: 'DELETE',
          });
  ...
  const handleSave = async () => {
    const url = isEditing
      ? `API_URL/adeco/api/update-provider.php`
      : `API_URL/adeco/api/create-provider.php`;
