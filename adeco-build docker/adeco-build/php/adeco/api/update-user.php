...
try {
    ...
    if (!empty($id_key) || !empty($tag)) {
        $whereParts = [];
        $params = [':current_dni' => $dni];

        if (!empty($id_key)) {
            $whereParts[] = 'Id_Key = :id_key';
        ...
        if (count($whereParts) > 0) {
            ...
            if ($existing) {
                $conflictField = '';
                if (!empty($id_key) && $existing['Id_Key'] === $id_key) $conflictField = 'La llave';
                elseif (!empty($tag) && $existing['tag'] === $tag) $conflictField = 'El tag';
